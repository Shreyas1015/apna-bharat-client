...
const AppliedApplicationsForRent = () => {
    ...
    const fetchAppliedApplicationsForRentedProducts = async () => {
      try {
        const res = await axiosInstance.post(
          `${process.env.REACT_APP_BASE_URL}/farmers/fetchAppliedApplicationsForRentedProducts`,
          { decryptedUID }
        );
        if (res.status === 200) {
  ...
  const handleAcceptApplication = async (erId, requestId) => {
    try {
      const res = await axiosInstance.post(
        `${process.env.REACT_APP_BASE_URL}/farmers/acceptEquipmentApplication`,
...
export default AppliedApplicationsForRent;
