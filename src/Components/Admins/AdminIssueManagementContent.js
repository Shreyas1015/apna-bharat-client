...
const AdminIssueManagementContent = () => {
    ...
    const getIssueData = async () => {
      try {
        const res = await axiosInstance.post(
          `${process.env.REACT_APP_BASE_URL}/admin/getIssueData`,
          { decryptedUID }
        );
        if (res.status === 200) {
  ...
  const handleStatusChange = async (issueId, newStatus) => {
    try {
      ...
      const res = await axiosInstance.post(
        `${process.env.REACT_APP_BASE_URL}/admin/updateIssueStatus`,
        {
          issueId,
          newStatus,
          decryptedUID,
...
export default AdminIssueManagementContent;
